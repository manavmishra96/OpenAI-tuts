...
def episode(env, param):
	...
	for _ in range(goal_steps):
		...
		chosen_action = np.random.choice([0,1], p = softmax(action_space))
...
def initial_population(param, goal_steps, min_reward, initial_games):
...
if __name__ == '__main__':
	param = np.random.rand(2,4)*2 - 1    #to keep it between -1 and 1
	LR = 1e-3
	goal_steps = 200
	min_reward = 50
	initial_games = 5000

	training_data = initial_population(param, goal_steps, min_reward, initial_games)
